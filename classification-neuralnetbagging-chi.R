...
dataset_metrics_name <- paste("nnetbagging-classification","-","dataset",iter,"-","chi","-","metrics",".","csv",sep="")
dataset_predictions_name <- paste("nnetbagging-classification","-","dataset",iter,"-","chi","-","predictions",".","csv",sep="")  
if (iter == 1 | iter == 6 | iter == 7 | iter == 8 | iter == 10) 
s = 3 
else
s = 1  
...
bagctrl <- bagControl(fit = nnetBag$fit,predict = nnetBag$pred,aggregate = nnetBag$aggregate)
model <- train(zclass ~ ., data = dataset,"bag",size=s,trControl = ctrl,bagControl = bagctrl)
