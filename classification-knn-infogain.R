...
for(iter in 1:10)
{
  ...
  dataset_infogain_name <- paste("dataset",iter,"-","infogain",".","arff",sep="")
  dataset_metrics_name <- paste("knn-classification","-","dataset",iter,"-","infogain","-","metrics",".","csv",sep="")
  dataset_predictions_name <- paste("knn-classification","-","dataset",iter,"-","infogain","-","predictions",".","csv",sep="")  
...
dataset <- read.arff(dataset_infogain_name)
temp <- cbind("infogain filtered Dimensions","")
write.table(temp,dataset_metrics_name,append=T,sep=",",eol = "\n",col.names=F,row.names=F)
