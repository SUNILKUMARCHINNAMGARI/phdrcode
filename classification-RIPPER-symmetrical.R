...
dataset_metrics_name <- paste("RIPPER-classification","-","dataset",iter,"-","symmetrical","-","metrics",".","csv",sep="")
dataset_predictions_name <- paste("RIPPER-classification","-","dataset",iter,"-","symmetrical","-","predictions",".","csv",sep="")  
...
model <- train(zclass ~ ., data = dataset, method = "JRip", trControl = ctrl)
NumOptvalue<-as.integer(model$bestTune[1])
modelpred <- model$pred
modelpred <- subset(modelpred,NumOpt==NumOptvalue)
