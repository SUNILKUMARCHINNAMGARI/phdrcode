...
for(iter in 1:10)
{
  ...
  dataset_metrics_name <- paste("knn-classification","-","dataset",iter,"-","pca",num,"-","metrics",".","csv",sep="")
  dataset_predictions_name <- paste("knn-classification","-","dataset",iter,"-","pca",num,"-","predictions",".","csv",sep="")  
...
model <- train(class ~ ., data = dataset, method = "knn",tuneGrid=expand.grid(.k=1:25),metric='Accuracy', trControl = ctrl)
