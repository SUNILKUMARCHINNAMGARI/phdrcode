...
for(iter in 1:10)
{
  ...
  dataset_gain_name <- paste("dataset",iter,"-","gain",".","arff",sep="")
  dataset_metrics_name <- paste("knn-classification","-","dataset",iter,"-","gain","-","metrics",".","csv",sep="")
  dataset_predictions_name <- paste("knn-classification","-","dataset",iter,"-","gain","-","predictions",".","csv",sep="")  
...
dataset <- read.arff(dataset_gain_name)
temp <- cbind("gain filtered Dimensions","")
write.table(temp,dataset_metrics_name,append=T,sep=",",eol = "\n",col.names=F,row.names=F)
