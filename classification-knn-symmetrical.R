...
for(iter in 1:10)
{
  ...
  dataset_symmetrical_name <- paste("dataset",iter,"-","symmetrical",".","arff",sep="")
  dataset_metrics_name <- paste("knn-classification","-","dataset",iter,"-","symmetrical","-","metrics",".","csv",sep="")
  dataset_predictions_name <- paste("knn-classification","-","dataset",iter,"-","symmetrical","-","predictions",".","csv",sep="")  
...
dataset <- read.arff(dataset_symmetrical_name)
temp <- cbind("symmetrical filtered Dimensions","")
write.table(temp,dataset_metrics_name,append=T,sep=",",eol = "\n",col.names=F,row.names=F)
dimensions <- cbind(dim(dataset)[1],(dim(dataset)[2]-1))
write.table(dimensions,dataset_metrics_name,append=T,sep=",",eol = "\n",col.names=F,row.names=F)
ctrl <- trainControl(method = "cv", number = 10,savePred = T)
model <- train(zclass ~ ., data = dataset, method = "knn",tuneGrid=expand.grid(.k=1:25),metric='Accuracy', trControl = ctrl)
kvalue<-as.integer(model$bestTune[1])
modelpred <- model$pred
modelpred <- subset(modelpred,k==kvalue)
write.csv(modelpred,dataset_predictions_name)
library(mlearning)
library(Metrics)
