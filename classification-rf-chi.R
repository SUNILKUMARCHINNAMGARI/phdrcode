...
dataset_metrics_name <- paste("rf-classification","-","dataset",iter,"-","chi","-","metrics",".","csv",sep="")
dataset_predictions_name <- paste("rf-classification","-","dataset",iter,"-","chi","-","predictions",".","csv",sep="")  
...
write.table(dimensions,dataset_metrics_name,append=T,sep=",",eol = "\n",col.names=F,row.names=F)
colcount <- sqrt(ncol(dataset)-1)
grid_rf <- expand.grid(.mtry = c((colcount/2), (colcount/4), (colcount*3/4), colcount,(colcount*colcount)))
ctrl <- trainControl(method = "cv", number = 10,savePred = T)
model <- train(zclass ~ ., data = dataset, method = "rf", trControl = ctrl,tuneGrid = grid_rf)
mtryvalue<-as.numeric(model$bestTune[1])
modelpred <- model$pred
#levels(modelvalue) <- levels(modelpred$model)
modelpred <- subset(modelpred,mtry==mtryvalue)
