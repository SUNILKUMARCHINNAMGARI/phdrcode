...
for(iter in 1:10)
{
  ...
  dataset_relief_name <- paste("dataset",iter,"-","relief",".","arff",sep="")
  dataset_metrics_name <- paste("knn-classification","-","dataset",iter,"-","relief","-","metrics",".","csv",sep="")
  dataset_predictions_name <- paste("knn-classification","-","dataset",iter,"-","relief","-","predictions",".","csv",sep="")  
...
dataset <- read.arff(dataset_relief_name)
temp <- cbind("relief filtered Dimensions","")
write.table(temp,dataset_metrics_name,append=T,sep=",",eol = "\n",col.names=F,row.names=F)
