...
for(iter in 1:10)
{
  for (num in c(0.90,0.92,0.94,0.96,0.98))
  {
    ...
    dataset_metrics_name <- paste("OneR-classification","-","dataset",iter,"-","pca",num,"-","metrics",".","csv",sep="")
    dataset_predictions_name <- paste("OneR-classification","-","dataset",iter,"-","pca",num,"-","predictions",".","csv",sep="")  
...
model <- train(class ~ ., data = dataset, method = "OneR", trControl = ctrl)
